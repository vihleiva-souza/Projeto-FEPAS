...
function updateProgressVisualization(progressData) {
  ...
  const planned = src.total_testes_planejados || src.total_planned || src.planned || 0;
  const approved = src.testes_aprovados || src.total_approved || src.approved || 0;
  const initiated = src.testes_iniciados || src.total_initiated || src.initiated || approved;
...
function clearWorkspaceCnpj() {
  ...
  if (cnpjInputAutorizador) cnpjInputAutorizador.value = "";
...
async function loadTests(productId) {
  const pid = productId || (typeof selectedProductId !== 'undefined' ? selectedProductId : "01");
  const url = `/api/produtos/${pid}/tests`;
...
async function loadClientProgress(cnpj, selectedTest, productId) {
  const normalized = normalizeCnpj(cnpj);
  // Progresso não depende do produto, apenas do CNPJ
  const url = `/api/client/progress?cnpj=${encodeURIComponent(normalized)}`;
  const resp = await fetch(url);
  const data = await resp.json();
  if (!resp.ok) {
...
changeCnpjBtn.addEventListener("click", () => {
  // Voltar para seleção de produto se a função existir
  if (typeof backToProductSelection !== 'undefined') {
...
accessForm.addEventListener("submit", async (ev) => {
  ...
  // Use a função de seleção de produto para obter o CNPJ do input correto
  const cnpjValue = typeof getCnpjFromForm !== "undefined"
    ? getCnpjFromForm()
  ...
  try {
    const productId = typeof selectedProductId !== "undefined" && selectedProductId
      ? selectedProductId
      : (localStorage.getItem("homolog_selected_product") || "01");

    await loadTests(productId);
    setWorkspaceCnpj(normalized);
    accessPanel.classList.add("hidden");
    await loadClientProgress(normalized, null, productId);
  } catch (err) {
    clearWorkspaceCnpj();
...
form.addEventListener("submit", async (ev) => {
  ...
  try {
    ...
    const pid = typeof selectedProductId !== "undefined" && selectedProductId
      ? selectedProductId
      : (localStorage.getItem("homolog_selected_product") || "01");
...
const savedProductId = typeof selectedProductId !== 'undefined' ? selectedProductId : localStorage.getItem("homolog_selected_product") || "01";
...
if (savedCnpj) {
  ...
  accessPanel.classList.add("hidden");
  loadClientProgress(savedCnpj, null, savedProductId).catch(() => {
    clearWorkspaceCnpj();
  });
}
